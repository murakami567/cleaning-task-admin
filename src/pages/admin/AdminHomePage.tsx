...
function buildMonthCells(year: number, month: number) {
  ...
  for (let i = 0; i < startWeekday; i++) {
    ...
    cells.push({ date: toDateString(d), day: d.getDate(), inMonth: false });
  }

  for (let day = 1; day <= lastDate; day++) {
    const d = new Date(year, month - 1, day);
    cells.push({ date: toDateString(d), day, inMonth: true });
  }
...
function isAfterMissingClockInTime() {
  const now = new Date();
  const threshold = new Date(now);
  threshold.setHours(MISSING_CLOCK_IN_HOUR, MISSING_CLOCK_IN_MINUTE, 0, 0);
  return now.getTime() >= threshold.getTime();
}

function getClockInTime(entry: ShiftEntry) {
  return entry.clock_in_at || entry.attendance?.clock_in_at || "";
}

function isMissingClockIn(entry: ShiftEntry, afterThreshold: boolean) {
  if (!afterThreshold) return false;

  const status = entry.attendance_status || entry.attendance?.status || "";
...
export default function AdminHomePage() {
  ...
  const afterMissingClockInTime = useMemo(() => {
    void nowTick;
    return isAfterMissingClockInTime();
  }, [nowTick]);

  const missingClockInCount = useMemo(() => {
    return (todayShift?.shift_entries || []).filter((entry) =>
      isMissingClockIn(entry, afterMissingClockInTime)
    ).length;
  }, [todayShift, afterMissingClockInTime]);
  ...
  async function saveMessage() {
    try {
      ...
      const res = await authorizedFetch(`${API_BASE}/api/admin-portal/today-message`, {
        ...
        body: JSON.stringify({ target_date: draftDate, message: draftMessage }),
      });
      const data = await res.json();
  ...
  function toggleDraftAssignee(staff: Staff) {
    setScheduleDraft((prev) => {
      ...
      if (exists) {
        return {
          ...prev,
          assignee_ids: prev.assignee_ids.filter((id) => id !== staff.id),
          assignee_names: prev.assignee_names.filter((name) => name !== staff.staff_name),
        };
      }
      return {
        ...prev,
  ...
  async function saveSchedule() {
    try {
      ...
      if (!scheduleDraft.start_date) throw new Error("開始日は必須です。");
      if (!scheduleDraft.end_date) throw new Error("終了日は必須です。");
      if (scheduleDraft.end_date < scheduleDraft.start_date) {
        throw new Error("終了日は開始日以降にしてください。");
      }
      if (!scheduleDraft.title.trim()) throw new Error("タイトルは必須です。");
      ...
      const res = scheduleEditMode === "edit" && scheduleDraft.id
        ? await authorizedFetch(`${API_BASE}/api/admin-portal/calendar-schedules/${scheduleDraft.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body,
          })
        : await authorizedFetch(`${API_BASE}/api/admin-portal/calendar-schedules`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body,
          });
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div
            onClick={() => window.open("https://order-management-hoq5.onrender.com","_blank","noopener,noreferrer")}
          ...
          <div
            onClick={() => window.open("https://gusk-property-management.onrender.com/opening","_blank","noopener,noreferrer")}
            className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm cursor-pointer hover:-translate-y-0.5 hover:shadow-md transition">
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  10:05以降、Jinjer出勤打刻がないスタッフを未打刻表示します。
                </p>
              </div>
              <div className="flex items-center gap-2">
                {missingClockInCount > 0 ? (
                  <span className="rounded-full bg-rose-600 px-3 py-0.5 text-xs font-bold text-white">
                    未打刻 {missingClockInCount} 名
                  </span>
                ) : null}
                <span className="rounded-full bg-slate-100 px-3 py-0.5 text-xs font-bold text-slate-700">
                  {todayShift?.shift_entries?.length ?? 0} 名
                </span>
              </div>
            </div>

            <div className="mt-4 max-h-[480px] overflow-y-auto pr-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {!todayShift || !todayShift.shift_entries || todayShift.shift_entries.length === 0 ? (
                <div className="col-span-full rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-500">
                  本日のシフトはありません
                </div>
              ) : (
                todayShift.shift_entries.map((entry) => {
                  const missing = isMissingClockIn(entry, afterMissingClockInTime);
                  const clockIn = getClockInTime(entry);
                  return (
                    <div
                      key={entry.id}
        ...
        >
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-xs text-slate-500">{onBreakStaff.length} 名 / 30秒ごとに更新</span>
          </div>
          <div className="mt-4">
            {onBreakStaff.length === 0 ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {onBreakStaff.map((s) => (
                  <div key={s.id} className="rounded-2xl border border-amber-300 bg-white px-4 py-3">
                    <div className="font-bold text-slate-900">{s.name || "-"}</div>
                    <div className="mt-1 text-xs text-slate-500">
                      {s.staff_code ? `${s.staff_code} / ` : ""}{s.role || "-"}
                    </div>
                    <div className="mt-2 text-sm text-amber-700">{formatBreakSince(s.break_started_at)}</div>
                  </div>
                ))}
              </div>
            )}
      ...
      {messageModalOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 px-4" onMouseDown={(e) => { if (e.target === e.currentTarget) setMessageModalOpen(false); }}>
          <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <div className="mt-4 space-y-4">
              <div>
                ...
                <input type="date" value={draftDate} onChange={(e) => setDraftDate(e.target.value)} className="h-11 w-full rounded-2xl border border-slate-200 px-4" />
              </div>
              <div>
                <div className="mb-2 text-sm font-semibold text-slate-700">内容</div>
                <textarea value={draftMessage} onChange={(e) => setDraftMessage(e.target.value)} rows={8} className="w-full rounded-2xl border border-slate-200 p-4 text-sm outline-none" placeholder="表示する内容を入力" />
              </div>
            </div>
            <div className="mt-5 flex gap-3">
              <button onClick={() => setMessageModalOpen(false)} className="flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">キャンセル</button>
              <button onClick={saveMessage} disabled={savingMessage} className="flex-1 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-bold text-white disabled:opacity-50">{savingMessage ? "保存中..." : "保存"}</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {scheduleModalOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 px-4" onMouseDown={(e) => { if (e.target === e.currentTarget) setScheduleModalOpen(false); }}>
          <div className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <h3 className="text-xl font-bold text-slate-900">{scheduleEditMode === "edit" ? "スケジュール編集" : "スケジュール追加"}</h3>
              {scheduleEditMode === "edit" && scheduleDraft.id ? (
                <button onClick={deleteSchedule} disabled={deletingSchedule} className="rounded-2xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-100 disabled:opacity-50">{deletingSchedule ? "削除中..." : "削除"}</button>
              ) : null}
            </div>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <div>
                <div className="mb-2 text-sm font-semibold text-slate-700">開始日</div>
                <input type="date" value={scheduleDraft.start_date} onChange={(e) => setScheduleDraft((prev) => ({ ...prev, start_date: e.target.value }))} className="h-11 w-full rounded-2xl border border-slate-200 px-4" />
              </div>
              <div>
                <div className="mb-2 text-sm font-semibold text-slate-700">終了日</div>
                <input type="date" value={scheduleDraft.end_date} onChange={(e) => setScheduleDraft((prev) => ({ ...prev, end_date: e.target.value }))} className="h-11 w-full rounded-2xl border border-slate-200 px-4" />
              </div>
            </div>
            <div className="mt-4">
              <div className="mb-2 text-sm font-semibold text-slate-700">タイトル</div>
              <input type="text" value={scheduleDraft.title} onChange={(e) => setScheduleDraft((prev) => ({ ...prev, title: e.target.value }))} className="h-11 w-full rounded-2xl border border-slate-200 px-4" placeholder="タイトルを入力" />
            </div>
            <div className="mt-4">
              ...
              <div className="max-h-56 space-y-2 overflow-auto rounded-2xl border border-slate-200 bg-slate-50 p-3">
                {staffs.length === 0 ? (
                  ...
                  staffs.map((staff) => {
                    ...
                    return (
                      <label key={staff.id} className="flex items-center gap-3 rounded-xl bg-white px-3 py-2 text-sm text-slate-700">
                        <input type="checkbox" checked={checked} onChange={() => toggleDraftAssignee(staff)} />
              ...
              <div className="mt-2 text-sm text-slate-500">
                {scheduleDraft.assignee_names.length > 0 ? `選択中: ${scheduleDraft.assignee_names.join(" / ")}` : "担当者未設定"}
              </div>
            </div>
            <div className="mt-4">
              <div className="mb-2 text-sm font-semibold text-slate-700">内容</div>
              <textarea value={scheduleDraft.description} onChange={(e) => setScheduleDraft((prev) => ({ ...prev, description: e.target.value }))} rows={6} className="w-full rounded-2xl border border-slate-200 p-4 text-sm outline-none" placeholder="内容を入力" />
            </div>
            <div className="mt-5 flex gap-3">
              <button onClick={() => setScheduleModalOpen(false)} className="flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">キャンセル</button>
              <button onClick={saveSchedule} disabled={savingSchedule} className="flex-1 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-bold text-white disabled:opacity-50">{savingSchedule ? "保存中..." : scheduleEditMode === "edit" ? "更新" : "保存"}</button>
            </div>
          </div>
        </div>
      ) : null}
