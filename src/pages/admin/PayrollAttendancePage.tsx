                        ...
                        {open ? <tr className="border-t bg-neutral-50"><td colSpan={10} className="p-4">
                          <div className="overflow-auto rounded-xl border bg-white">
                            <table className="w-full min-w-[1050px] text-xs">
                              <thead className="bg-neutral-50 text-neutral-500"><tr><th className="px-3 py-2 text-left">施設</th><th className="px-3 py-2 text-left">完了部屋・時間内訳</th><th className="px-3 py-2 text-right">部屋数</th><th className="px-3 py-2 text-right">清掃報酬</th><th className="px-3 py-2 text-right">実働</th><th className="px-3 py-2 text-right">時給報酬</th><th className="px-3 py-2 text-right">保証調整</th><th className="px-3 py-2 text-right">交通費</th><th className="px-3 py-2 text-right">支給額</th><th className="px-3 py-2 text-right">操作</th></tr></thead>
                              <tbody>{group.rows.map((r) => <tr key={r.id} className="border-t"><td className="px-3 py-3 font-medium">{r.facility || "-"}</td><td className="max-w-[360px] px-3 py-3 leading-5 text-neutral-600">{r.note || "-"}</td><td className="px-3 py-3 text-right">{r.room_count || 0}</td><td className="px-3 py-3 text-right">{yen(r.cleaning_amount)}</td><td className="px-3 py-3 text-right">{Number(r.actual_hours || 0).toFixed(2)}h</td><td className="px-3 py-3 text-right">{yen(r.hourly_amount)}</td><td className="px-3 py-3 text-right">{yen(r.adjustment_amount)}</td><td className="px-3 py-3 text-right">{yen(r.transportation_fee)}</td><td className="px-3 py-3 text-right font-semibold">{yen(r.final_amount)}</td><td className="px-3 py-3"><div className="flex justify-end gap-2"><button type="button" onClick={(e) => { e.stopPropagation(); setEditingPayroll(r); }} className="rounded-lg border bg-white px-3 py-1.5 font-medium">修正</button><button type="button" onClick={(e) => { e.stopPropagation(); void deletePayrollResult(r); }} className="rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 font-medium text-red-600">削除</button></div></td></tr>)}</tbody>
                            </table>
                          </div>
                        </td></tr> : null}
                      </React.Fragment>;
  ...
  return (
    <Card className="print-area p-5">
      ...
      {!selectedStaff ? <div className="rounded-2xl border bg-neutral-50 p-6 text-sm text-neutral-500">対象スタッフがありません</div> : <>
        ...
        <div className="overflow-auto rounded-2xl border"><table className="w-full min-w-[900px] text-sm"><thead className="bg-neutral-100 text-xs text-neutral-600"><tr><th className="px-3 py-3 text-left">日付</th><th className="px-3 py-3 text-left">施設</th><th className="px-3 py-3 text-left min-w-[280px]">完了部屋・時間内訳</th><th className="px-3 py-3 text-right">部屋数</th><th className="px-3 py-3 text-right">清掃報酬</th><th className="px-3 py-3 text-right">実働</th><th className="px-3 py-3 text-right">時給報酬</th><th className="px-3 py-3 text-right">保証調整</th><th className="px-3 py-3 text-right">交通費</th><th className="px-3 py-3 text-right">支給額</th></tr></thead><tbody>{rows.map((r: PayrollDailyResult) => { const displayDate = previousDate !== r.target_date; previousDate = r.target_date; return <tr key={r.id} className="border-t bg-white"><td className="px-3 py-3 font-medium">{displayDate ? formatMd(r.target_date) : ""}</td><td className="px-3 py-3">{r.facility}</td><td className="px-3 py-3 text-xs leading-5 text-neutral-600">{r.note || "-"}</td><td className="px-3 py-3 text-right">{r.room_count || ""}</td><td className="px-3 py-3 text-right">{yen(r.cleaning_amount)}</td><td className="px-3 py-3 text-right">{r.actual_hours ? `${r.actual_hours}h` : ""}</td><td className="px-3 py-3 text-right">{yen(r.hourly_amount)}</td><td className="px-3 py-3 text-right">{yen(r.adjustment_amount)}</td><td className="px-3 py-3 text-right">{yen(r.transportation_fee)}</td><td className="px-3 py-3 text-right font-semibold">{yen(r.final_amount)}</td></tr>; })}</tbody><tfoot className="border-t-2 bg-neutral-50 font-semibold"><tr><td className="px-3 py-3" colSpan={3}>合計</td><td className="px-3 py-3 text-right">{total("room_count", rows)}</td><td className="px-3 py-3 text-right">{yen(total("cleaning_amount", rows))}</td><td className="px-3 py-3 text-right">{total("actual_hours", rows).toFixed(2)}h</td><td className="px-3 py-3 text-right">{yen(total("hourly_amount", rows))}</td><td className="px-3 py-3 text-right">{yen(total("adjustment_amount", rows))}</td><td className="px-3 py-3 text-right">{yen(total("transportation_fee", rows))}</td><td className="px-3 py-3 text-right">{yen(total("final_amount", rows))}</td></tr></tfoot></table></div>
      </>}
    </Card>
  );
}
