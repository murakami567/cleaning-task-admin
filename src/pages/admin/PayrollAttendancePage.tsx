    ...
    <div className="min-h-screen bg-neutral-50 p-6 text-neutral-900">
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Button active={tab === "daily"} variant={tab === "daily" ? "default" : "outline"} onClick={() => setTab("daily")}>当月確認一覧</Button>
            ...
            <Card className="p-5">
              ...
              <div className="overflow-auto rounded-2xl border">
                <table className="w-full min-w-[1100px] text-sm">
                  <thead className="bg-neutral-100 text-xs text-neutral-600"><tr><th className="px-3 py-3 text-left">日付</th><th className="px-3 py-3 text-left">スタッフ</th><th className="px-3 py-3 text-left">計算方式</th><th className="px-3 py-3 text-left">施設</th><th className="px-3 py-3 text-left min-w-[300px]">完了部屋・時間内訳</th><th className="px-3 py-3 text-right">部屋数</th><th className="px-3 py-3 text-right">清掃報酬</th><th className="px-3 py-3 text-right">実働</th><th className="px-3 py-3 text-right">時給報酬</th><th className="px-3 py-3 text-right">保証調整</th><th className="px-3 py-3 text-right">交通費</th><th className="px-3 py-3 text-right">支給額</th><th className="px-3 py-3 text-left">状態</th></tr></thead>
                  <tbody>{rows.map((r) => <tr key={r.id} className="cursor-pointer border-t bg-white hover:bg-neutral-50" onClick={() => { setSelectedStaffId(r.staff_id); setTab("statement"); }}><td className="px-3 py-3">{formatMd(r.target_date)}</td><td className="px-3 py-3 font-medium">{r.staff_name}</td><td className="px-3 py-3"><Pill>{typeLabel(r.payroll_type)}</Pill></td><td className="px-3 py-3">{r.facility}</td><td className="px-3 py-3 text-xs leading-5 text-neutral-600">{r.note || "-"}</td><td className="px-3 py-3 text-right">{r.room_count || ""}</td><td className="px-3 py-3 text-right">{yen(r.cleaning_amount)}</td><td className="px-3 py-3 text-right">{r.actual_hours ? `${r.actual_hours}h` : ""}</td><td className="px-3 py-3 text-right">{yen(r.hourly_amount)}</td><td className="px-3 py-3 text-right">{yen(r.adjustment_amount)}</td><td className="px-3 py-3 text-right">{yen(r.transportation_fee)}</td><td className="px-3 py-3 text-right font-semibold">{yen(r.final_amount)}</td><td className="px-3 py-3"><Pill tone={r.status === "確定" ? "good" : "warn"}>{r.status || "未確定"}</Pill></td></tr>)}</tbody>
                </table>
              </div>
            </Card>
          </div>
