...
export default function CompanyCalendarPage() {
  ...
  return <div className="p-4 sm:p-6">
      ...
      <div className="mt-5 overflow-x-auto">
        <div className="min-w-[1050px] grid grid-cols-7 gap-2">
          ...
          {cells.map(cell => {
            ...
            const dayMessages = messages.filter(x => x.target_date === cell.date);
            const cleaningCount = cleaningCounts[cell.date] || 0;
    ...
    {selectedDate ? <div className="fixed inset-0 z-[900] flex items-end bg-black/30 md:items-center md:justify-center md:px-4" onMouseDown={e => { if (e.target === e.currentTarget) setSelectedDate(null); }}>
      <div className="max-h-[85vh] w-full overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl md:max-w-2xl md:rounded-3xl">
        ...
        <section className="mt-5"><h4 className="font-bold text-slate-900">連絡事項</h4><div className="mt-2 space-y-2">{selectedMessages.length ? selectedMessages.map(x => <div key={x.id} className="rounded-xl bg-violet-50 p-3 text-sm text-violet-950 whitespace-pre-wrap">{x.message}</div>) : <div className="text-sm text-slate-400">連絡事項はありません。</div>}</div></section>
      </div>
    </div> : null}
